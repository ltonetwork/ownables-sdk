...
export default function CreateOwnable(props: CreateOwnableProps) {
  ...
  const getOwnables = async () => {
    try {
      ...
      const response = await axios.get("http://localhost:3000/api/v1/requestIDs?onlyClaimable=true&ltoUserAddress=3NCfghPcoym62MrXj6To5uRkiFp4xNDi5LK");
  ...
  const getOwnable = async (ownable: {NAME: string, RID: string }) => {
    console.log("ownable", ownable);
    const getZip = await axios.get("http://localhost:3000/api/v1/claim/bafybeigcu67yh4ansel2mbjue6jupbpa2mvpyg262gt76qzau5bunppu7y");
    const downloadZip = async () => {
      const response = await axios.get("http://localhost:3000/api/v1/claim/bafybeigcu67yh4ansel2mbjue6jupbpa2mvpyg262gt76qzau5bunppu7y", {
        responseType: 'blob',
      });
          ...
          <Box>
            ...
            {activeTab === "import" && (
              <div>
                ...
                {ownables.map((readyOwnable, index) => (
                  <Grid
                    container
                    justifyContent="space-between"
                    alignItems="center"
                  ...
                  >
                    ...
                    <GridItem item xs={2}>
                      {/* <span>{readyOwnable.NFT_BLOCKCHAIN}</span> */}
                      {readyOwnable.NFT_BLOCKCHAIN === 'ethereum' && <img src="/ethereum-logo-grey.svg" alt="Ethereum logo" />}
